...
{
    echo 'error: bad args' . PHP_EOL;
    exit;
}
$users = intval($users);
if ($users <= 0 || $users > 1000)
{
    echo 'error: bad users' . PHP_EOL;
    exit;
}

$R_Codes = [
    0x39CB44B8, 0x23754F67, 0x5F017211, 0x3EBB24DA,
    0x351707C6, 0x63F9774B, 0x17827288, 0xFE74821,
    ...
    0x15CC060B, 0xAFD13CC,  0x28603BCF, 0x3371066B,
    ...
    0x41A069D6, 0x25400192, 0xDD4667,   0x6AFC1F4F,
    ...
    0xF42094B,  0x2FD820E6, 0x778B2E5E, 0x71176D02,
    0x7FEA7A69, 0x5BB54628, 0x19BA6C71, 0x39763A99,
    0x178D54CD, 0x1246E88,  0x3313537E, 0x2B8E2D17,
    0x2A3D10BE, 0x59D10582, 0x37A163DB, 0x30D6489A,
    0x6A215C46, 0xE1C7A76,  0x1FC760E7, 0x79B80C65,
    ...
    0x637641D8, 0x33352A42, 0x51A22C19, 0x85C5851,
    0x32917AB,  0x2B770AC7, 0x30AC77B3, 0x2BEC1907,
    0x35202D0,  0xFA933D3,  0x61255DF3, 0x22AD06BF,
    ...
    0x4A000D49, 0x57AE1C70, 0x4E2477,   0x561E7E72,
    0x468C0033, 0x5DCC2402, 0x78507AC6, 0x58AF24C7,
    0xDF62D34,  0x358A4708, 0x3CFB1E11, 0x2B71451C,
    0x77A75295, 0x56890721, 0xFEF75F3,  0x120F24F1,
    0x1990AE7,  0x339C4452, 0x27A15B8E, 0xBA7276D,
    ...
    0xBA85EC9,  0x6ACA6B46, 0x1E43422A, 0x514D5F0E,
    0x413E018C, 0x307626E9, 0x1ED1DFA,  0x49F46F5A,
    ...
    0x257B5DF4, 0xA5C2C5B,  0x16A45527, 0x16F23945
];
function R_EncodeName($name, $users)
{
    global $R_Codes;
    ...
    {
        $ch = ord($name[$i]);
        $result = ($result + $R_Codes[$ch]) & 0xffffffff;
        $result = ($result ^ $R_Codes[($ch + 13) & 0xff]) & 0xffffffff;
        $result = ($result * $R_Codes[($ch + 47) & 0xff]) & 0xffffffff;
        $result = ($result + $R_Codes[($i * 9) & 0xff]) & 0xffffffff;
        $result = ($result + $R_Codes[($i * 19) & 0xff]) & 0xffffffff;
        $result = ($result + $R_Codes[($users * 15 + $i * 13) & 0xff]) & 0xffffffff;
    }
    return $result;
}

function R_EncodeUsers($users)
{
    $users = ($users * 11) & 0xffff;
    $users = ($users ^ 0x3421) & 0xffff;
    $users = ($users + 0xb2d0) & 0xffff;
    $users = ($users ^ 0x7892) & 0xffff;
    return $users;
}

function R_EncodeVersion($version)
{
    $version = ($version ^ 0xa7) & 0xff;
    $version = ($version - 0x3d) & 0xff;
    $version = ($version ^ 0x18) & 0xff;
    return $version;
}

$name_encoded = R_EncodeName(strtoupper($name) , $users);
$users_encoded = R_EncodeUsers($users);
$version_encoded = R_EncodeVersion($version);

$code7 = ($name_encoded >> 24) & 0xff;
$code6 = ($name_encoded >> 16) & 0xff;
$code5 = ($name_encoded >> 8) & 0xff;
$code4 = $name_encoded & 0xff;
$code3 = 0x9c;
$code12 = ($code7 * 0x100 + $code5) ^ $users_encoded;
$code2 = $code12 & 0xff;
$code1 = ($code12 >> 8) & 0xff;
$code0 = $code6 ^ $version_encoded;
